import {
  ...
  sanitizeSummaryLine,
  validateSummaryTriple,
  type NewsSummaryTriple,
...
export async function generateNewsSummary(params: {
  title: string;
  snippet: string;
}): Promise<{ summary: NewsSummaryTriple; category: ESGCategory }> {
  ...
  const fallbackSummary = buildTemplateSummary(title, snippet);
  return {
...
async function requestAiSummary(
  ...
  title: string,
  snippet: string,
): Promise<{ summary: NewsSummaryTriple; category: ESGCategory } | null> {
  ...
  try {
    ...
    const summary = validateSummaryTriple(parsed?.summary);
    if (!summary || !isValidCategory) return null;

    return {
      summary: summary.map((line) => sanitizeSummaryLine(line)) as NewsSummaryTriple,
      category,
    };
  } catch {
    return null;
...
export async function generateNewsSummaries(
  ...
  concurrency = 3,
): Promise<Array<{ summary: NewsSummaryTriple; category: ESGCategory }>> {
  const results: Array<{ summary: NewsSummaryTriple; category: ESGCategory }> = [];

  for (let i = 0; i < items.length; i += concurrency) {
    const chunk = items.slice(i, i + concurrency);
    const chunkResults = await Promise.all(
      chunk.map((item) => generateNewsSummary({ title: item.title, snippet: item.snippet })),
    );
